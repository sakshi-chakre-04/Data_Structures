...
//  Description :       It is used to delete all files
//  Input :             Filename
//  Output :            Nothing
//  Author :            Sakshi Chakre
//  Date :              22/01/2026
              ...
              )   
{
    ...
    for(i = 0; i < MAXOPENFILES; i++)
    {
        if(uareaobj.UFDT[i] != NULL)
        {
            if(strcmp(name,uareaobj.UFDT[i]->ptrinode->FileName) == 0)
            {
                ...
                // Don't deallocate memory of the inode
                ...
                break;  // IMP : If we don't break, then it will delete all the files
            }// End of if
        }    // End of if
    }        // End of for
...
int main()
{
    char str[80] = {'\0'};
    char Command[5][20] = {{'\0'}};
    char InpuutBuffer[MAXFILESIZE] = {'\0'};
    ...
    while(1)
    {
        ...
        else if(iCount == 2)
        {
            ...
            else if(strcmp("unlink",Command[0]) == 0)
            {
                ...
                {
                    printf("Error : Unable to delete the file as parameters are invalid\n");
                }

                if(iRet == ERR_FILE_NOT_EXIST)
                {
                    printf("Error : Unable to delete the file as the file is not present\n");
                }
            ...
            else if(strcmp("write",Command[0]) == 0)
            {
                ...
                fgets(InpuutBuffer,MAXFILESIZE,stdin);

                iRet = WriteFile(atoi(Command[1]),InputBuffer,strlen(InputBuffer)-1);

                if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error : Unable to write the data as parameters are invalid\n");
                }
